\echo "bank_drafts_from_receipt(radiologist).sql loaded"
...
-- bank_drafts - radiologist(not dummy)
DO $$
    DECLARE
        v_log_id bigint;
    BEGIN
        select start_migration_log('bank_drafts_from_receipt(radiologist).sql - bank drafts(non-dummy)') into v_log_id;
        commit;
        ...
        select rec_detail.payment_no, sp.id, 'Radiologist',
        ...
        join fomema_backup_nios.radiologist_registration sp_reg on rec.tranno = sp_reg.tranno
        join fomema_backup_nios.radiologist_master sp_mas on sp_reg.rdregid = sp_mas.rdregid
        join radiologists sp on sp_mas.radiologist_code = sp.code
        ...
        where rec.service_type = 'RR' and rec.creation_date >= '2015-04-01';

        perform end_migration_log(v_log_id);
    END
$$;

\echo "bank_drafts_from_receipt(radiologist).sql - non-dummy done"

-- bank_drafts - radiologist (dummy)
DO $$
    DECLARE
        dummy_id bigint;
        v_log_id bigint;
    BEGIN
        select start_migration_log('bank_drafts_from_receipt(radiologist).sql - bank drafts(dummy)') into v_log_id;
        commit;

        SELECT id into dummy_id from radiologists where code = 'R123456789';
        ...
        select rec_detail.payment_no, dummy_id, 'Radiologist',
        banks.id, rec_detail.date_issue, rec_detail.branch_area, 
        rec_detail.payment_amount, rec_detail.payment_amount,
        case when rec.status = 'U' then false else true end,
        false, 
        ...
        rec.tranno, true
        from fomema_backup_nios.receipt rec
        left join fomema_backup_nios.radiologist_registration sp_reg USING (tranno)
        join fomema_backup_nios.receipt_detail rec_detail on rec.tranno = rec_detail.tranno
        join banks on rec_detail.bank_code = banks.code
        left join fomema_backup_nios.v_user_master creators on rec.creator_id = creators.uuid 
        left join users creator_users on creators.userid = creator_users.code 
        ...
        join organizations org on rec.branch_code = org.code
        join zones on rec_detail.zone_code = zones.code
        where rec.service_type = 'RR' and sp_reg.tranno is null and rec.creation_date >= '2015-04-01';

        perform end_migration_log(v_log_id);
    END
$$;
\echo "bank_drafts_from_receipt(radiologist).sql - dummy done"

\echo "bank_drafts_from_receipt(radiologist).sql ended"

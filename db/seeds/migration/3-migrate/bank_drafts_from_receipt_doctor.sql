\echo "bank_drafts_from_receipt(doctor).sql loaded"
...
-- bank_drafts - doctor(not dummy)
DO $$
    DECLARE
        v_log_id bigint;
    BEGIN
        select start_migration_log('bank_drafts_from_receipt(doctor).sql - bank drafts(non-dummy)') into v_log_id;
        commit;
        ...
        select rec_detail.payment_no, doc.id, 'Doctor',
        ...
        join fomema_backup_nios.doctor_registration doc_reg on rec.tranno = doc_reg.tranno
        join fomema_backup_nios.doctor_master doc_mas on doc_reg.dregid = doc_mas.dregid
        join doctors doc on doc_mas.doctor_code = doc.code
        ...
        where rec.service_type = 'DR' and rec.creation_date >= '2015-04-01';

        perform end_migration_log(v_log_id);
    END
$$;

\echo "bank_drafts_from_receipt(doctor).sql - non-dummy done"

-- bank_drafts - doctor (dummy)
DO $$
    DECLARE
        dummy_id bigint;
        v_log_id bigint;
    BEGIN
        select start_migration_log('bank_drafts_from_receipt(doctor).sql - bank drafts(dummy)') into v_log_id;
        commit;
        ...
        rec.tranno, true
        from fomema_backup_nios.receipt rec
        left join fomema_backup_nios.doctor_registration doc_reg USING (tranno)
        join fomema_backup_nios.receipt_detail rec_detail on rec.tranno = rec_detail.tranno
        join banks on rec_detail.bank_code = banks.code
        left join fomema_backup_nios.v_user_master creators on rec.creator_id = creators.uuid 
        left join users creator_users on creators.userid = creator_users.code 
        ...
        join organizations org on rec.branch_code = org.code
        join zones on rec_detail.zone_code = zones.code
        where rec.service_type = 'DR' and doc_reg.tranno is null and rec.creation_date >= '2015-04-01';

        perform end_migration_log(v_log_id);
    END
$$;
\echo "bank_drafts_from_receipt(doctor).sql - dummy done"

\echo "bank_drafts_from_receipt(doctor).sql ended"

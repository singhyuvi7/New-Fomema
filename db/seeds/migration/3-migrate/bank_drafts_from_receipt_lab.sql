\echo "bank_drafts_from_receipt(lab).sql loaded"
...
-- bank_drafts - lab(not dummy)
DO $$
    DECLARE
        v_log_id bigint;
    BEGIN
        select start_migration_log('bank_drafts_from_receipt(lab).sql - bank drafts(non-dummy)') into v_log_id;
        commit;
        ...
        select rec_detail.payment_no, sp.id, 'Laboratory',
        ...
        join fomema_backup_nios.laboratory_registration sp_reg on rec.tranno = sp_reg.tranno
        join fomema_backup_nios.laboratory_master sp_mas on sp_reg.lregid = sp_mas.lregid
        join laboratories sp on sp_mas.laboratory_code = sp.code
        ...
        where rec.service_type = 'LR' and rec.creation_date >= '2015-04-01';

        perform end_migration_log(v_log_id);
    END
$$;

\echo "bank_drafts_from_receipt(lab).sql - non-dummy done"

-- bank_drafts - lab (dummy)
DO $$
    DECLARE
        dummy_id bigint;
        v_log_id bigint;
    BEGIN
        select start_migration_log('bank_drafts_from_receipt(lab).sql - bank drafts(dummy)') into v_log_id;
        commit;

        SELECT id into dummy_id from laboratories where code = 'L123456789';
        ...
        select rec_detail.payment_no, dummy_id, 'Laboratory',
        banks.id, rec_detail.date_issue, rec_detail.branch_area, 
        rec_detail.payment_amount, rec_detail.payment_amount,
        case when rec.status = 'U' then false else true end,
        false, 
        ...
        rec.tranno, true
        from fomema_backup_nios.receipt rec
        left join fomema_backup_nios.laboratory_registration sp_reg USING (tranno)
        join fomema_backup_nios.receipt_detail rec_detail on rec.tranno = rec_detail.tranno
        join banks on rec_detail.bank_code = banks.code
        left join fomema_backup_nios.v_user_master creators on rec.creator_id = creators.uuid 
        left join users creator_users on creators.userid = creator_users.code 
        ...
        join organizations org on rec.branch_code = org.code
        join zones on rec_detail.zone_code = zones.code
        where rec.service_type = 'LR' and sp_reg.tranno is null and rec.creation_date >= '2015-04-01';

        perform end_migration_log(v_log_id);
    END
$$;
\echo "bank_drafts_from_receipt(lab).sql - dummy done"

\echo "bank_drafts_from_receipt(lab).sql ended"

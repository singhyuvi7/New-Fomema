...
module Report
  # Summary worker certification reporting service
  class SummaryWorkerCertificationService
    ...
    def set_transactions
      ...
      Transaction
        .select("organization_id, sum(case when fw_gender = 'M' then 1 else 0 end) AS male_count, sum(case when fw_gender = 'F' then 1 else 0 end) AS female_count, certification_date::date, organization_id")
        .where('certification_date BETWEEN ? AND ?', @start_date.to_date.beginning_of_day, @end_date.to_date.end_of_day)
        .search_organization(@branch_id)
        .joins(:organization)
        .group('certification_date::date, organization_id, organizations.name')
        .order('organizations.name ASC, certification_date ASC')
    end
    ...
    def transaction_mapping(transaction)
      [
        transaction.certification_date.try(:strftime, '%d/%m/%Y'),

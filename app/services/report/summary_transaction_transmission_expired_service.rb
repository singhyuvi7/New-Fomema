...
module Report
  # Summary Transaction Expired With Examination reporting service
  class SummaryTransactionTransmissionExpiredService
    ...
    def set_transactions
      ...
      Transaction
        .select("organization_id, sum(case when fw_gender = 'M' then 1 else 0 end) AS male_count, sum(case when fw_gender = 'F' then 1 else 0 end) AS female_count, transmission_expired_at::date, organization_id")
        .where('medical_examination_date is not null and certification_date is null')
        .where.not(status: ['REJECTED', 'CANCELLED'])
        .where('transmission_expired_at BETWEEN ? AND ?', @start_date.to_date.beginning_of_day, @end_date.to_date.end_of_day)
        .where('transaction_date >= ?', '2020-12-01 00:00:00')
        .search_organization(@branch_id)
        .joins(:organization)
        .group('transmission_expired_at::date, organization_id, organizations.name')
        .order('organizations.name ASC, transmission_expired_at ASC')
    end
    ...
    def transaction_mapping(transaction)
      [
        transaction.transmission_expired_at.try(:strftime, '%d/%m/%Y'),

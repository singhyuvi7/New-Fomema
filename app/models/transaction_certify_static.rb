class TransactionCertifyStatic < ApplicationRecord
    # audited
    acts_as_paranoid
    ...
    def self.seed_transaction_certify_static(start_date, end_date)
        transactions    = Transaction.where(certification_date: start_date...end_date).order(:id).includes(:medical_examination, :doctor_examination, :xray_facility, :laboratory, :doctor, :employer, foreign_worker: [:job_type])
        ...
        transactions.each.with_index(1) do |transaction, index|
            ...
            transaction_certify_static  = TransactionCertifyStatic.find_or_initialize_by(transaction_id: transaction.id)
            next if transaction_certify_static.id? # Do not need to update, they should not change at all.
            ...
            parameters          = {
                ...
                result: transaction.final_result || transaction.doctor_examination.suitability,
                doc_result: transaction.doctor_examination.suitability,
                final_result: transaction.final_result,
                final_result_date: transaction.final_result_date,
            ...
            if transaction.physical_exam_not_done
                parameters.merge! TransactionCertifyStatic.condition_hash_map.map {|key, field| [key, 0] }.to_h
            else
                parameters.merge! TransactionCertifyStatic.condition_hash_map.map {|key, field| [key, exam.try(field) ? 1 : 0] }.to_h
            end

            transaction_certify_static.update(parameters)
        end
    end

...
class External::AgencyWorkersController < ExternalController
    ...
    before_action :set_foreign_worker, only: [:show, :edit, :update, :destroy, :revert, :revert_update, :edit_address, :update_address]
    ...
    # before_action -> { can_proceed?('Employer', @foreign_worker.try(:employer), @foreign_worker.latest_transaction_id)}, if: -> { @foreign_worker.has_valid_latest_transaction_registered_by_employer? }, only: [:edit,:update]

    before_action -> { set_fee_amount }, if: -> { @foreign_worker.code.present? }, only: [:edit, :update]

    def index

        has_any_parameter   = [:code, :passport, :country, :without_code ].map {|param_name| params[param_name].present? }.include?(true)
        ...
        if has_any_parameter
            ...
            query = query.where(employer_id: params[:employer_id])

            # query.all.each do |foreignWorker|
            #     if foreignWorker&.address1.blank? && foreignWorker&.approval_status != "UPDATE_PENDING_APPROVAL"
            #         flash.now[:error] = "#{foreignWorker.name} (#{foreignWorker.passport_number}) - Accommodation of foreign worker (Address) is incomplete. Kindly update by clicking on Amendment button to avoid any failure to your registration.";
            #     end
            # end
        ...
        @transaction_carts = TransactionCart.select("foreign_workers.id, foreign_workers.name, foreign_workers.passport_number, foreign_workers.code, employers.code as empcode, employers.name as empname").joins(:foreign_worker).joins("join employers on foreign_workers.employer_id = employers.id").order("employers.code").where(created_by: current_user.id)
    end
    ...
    def create
        ...
        if ForeignWorker.where("passport_number = ? and status = 'ACTIVE'", foreign_worker_params[:passport_number]).count > 0
            ...
            if fw_matched.count == 1 && !fw_matched.first.is_reg_medical_blocked
                if fw_matched.first.employer.id == @employer.id
        ...
        respond_to do |format|
            if saveOK
                format.html { redirect_to external_agency_workers_url(employer_id: @employer.id, country: @foreign_worker.country_id, passport: @foreign_worker.passport_number), notice: 'Foreign worker data was successfully created. Kindly select the worker you wish to register and click on Add To List button.' }
    ...
    def update_address
        ...
        @foreign_worker.transaction do
            ...
            if ForeignWorker.where("passport_number = ? and status = 'ACTIVE' and id != ?", foreign_worker_params[:passport_number], @foreign_worker.id).count > 0
                @foreign_worker.errors.add(:passport_number, "existed, registration not allowed, please proceed to FOMEMA branch for registrations")
            ...
            if !@foreign_worker.valid?
                ...
                redirect_to external_agency_workers_path(employer_id: @employer.id, country: @foreign_worker.country_id, passport: @foreign_worker.passport_number) and return
        end
        # /db-transaction
    end
        ...
        @foreign_worker.transaction do
            ...
            if !@foreign_worker.code.blank?
                if !params[:foreign_worker].has_key?(:uploads) || params[:foreign_worker][:uploads].count == 0 || params[:foreign_worker][:uploads][0][:documents].blank?
                    # if !@foreign_worker.amendment_reasons == "Address"
            ...
            if [@foreign_worker.name_changed?, @foreign_worker.date_of_birth_changed?, @foreign_worker.country_id_changed?, @foreign_worker.passport_number_changed?, @foreign_worker.plks_number_changed?, @foreign_worker.arrival_date_changed?, @foreign_worker.maid_online_changed?, @foreign_worker.job_type_id_changed?, @foreign_worker.gender_changed?].none?
                # @foreign_worker.address1_changed?, @foreign_worker.address2_changed?, @foreign_worker.address3_changed?,  @foreign_worker.address4_changed?, @foreign_worker.state_id_changed?,  @foreign_worker.postcode_changed?,  @foreign_worker.town_id_changed?].none?
    ...
    def revert_update
        ...
        redirect_to external_agency_workers_path(employer_id: @employer.id), notice: "Reverted worker amendment request submitted, approval is required."
    end
    ...
    def change_gender
        ...
        approval_update_request(@foreign_worker, category: "FOREIGN_WORKER_GENDER_AMENDMENT", additional_information: approval_request_additional_information)
    ...
    def bulk_action
        if !params.has_key?(:ids)
            redirect_to external_agency_workers_path(employer_id: @employer.id), flash: {notice: "Please select worker"} and return
        end
        ...
        if @redirect_to != false
            redirect_to @redirect_to || external_agency_workers_path(employer_id: @employer.id)
        end
    end
    ...
    def change_employer
        ...
        respond_to do |format|
            format.html { redirect_to external_agency_workers_url(employer_id: @employer.id), notice: message }
            format.json { render :show, status: :created, location: foreign_worker }
        end
    end
    ...
    def set_foreign_worker
        ...
        }).first or redirect_to external_agency_workers_path, flash: {notice: "Worker not found"}
    end
    ...
    def set_agency
        @agency = Agency.find(current_user.userable_id)
    end

    def foreign_worker_params
        params.require(:foreign_worker).permit(:name, :gender, :date_of_birth, :passport_number, :country_id, :job_type_id, :plks_number, :address1, :address2, :address3, :address4, :country_id, :state_id, :postcode, :town_id, :pati, :amendment_reason_comment, :arrival_date, :employer_amended_at)
    end
    ...
    def set_fee_amount
        ...
        if @param_gender != @foreign_worker.gender && (!['CERTIFIED','CANCELLED','REJECTED'].include?(@foreign_worker.try(:latest_transaction).try(:status)) && @foreign_worker.try(:latest_transaction).try(:medical_examination_date).blank? && !@transaction_expired )

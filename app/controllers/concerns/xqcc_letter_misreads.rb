module XqccLetterMisreads
    def xqcc_misread_letter_index
        ...
        @letters    = XqccLetterMisread.search_by_xray_code(params[:xray_code])
        ...
        @previews   = {
            "Xray"      => xqcc_misread_letter_internal_xqcc_letter_path(":id_placeholder", type: "xray"),
            "Employer"  => xqcc_misread_letter_internal_xqcc_letter_path(":id_placeholder", type: "employer")
        }

        default_preview_page
    end

    def xqcc_misread_letter_new
        @letter = XqccLetterMisread.new
    end

    def xqcc_misread_letter_edit
        @letter = XqccLetterMisread.find_by(id: params[:id])
        render action_name.gsub("_edit", "_new")
    end

    def xqcc_misread_letter_save
        @letter = XqccLetterMisread.find_by(id: params[:xqcc_letter_misread][:id]) if params[:xqcc_letter_misread][:id].present?
        @letter ||= XqccLetterMisread.new
        @letter.assign_attributes(xqcc_letter_misread_params)
        ...
        when "Preview Xray Letter", "Preview Employer Letter"
            ...
            preview_xqcc_misread_letter
        else
            notice = @letter.save ? { notice: "Letter Saved" } : { error: "Something went wrong. Debug code: WTLS" }
            redirect_to xqcc_misread_letter_index_internal_xqcc_letters_path, notice
        end
    end

    def xqcc_misread_letter_search
        ...
        }.transform_keys {|key| "xqcc_letter_misread_#{ key }" }

        render json: results, status: :ok
    end

    def xqcc_misread_letter
        @letter = XqccLetterMisread.find_by(id: params[:id])
        redirect_to xqcc_misread_letter_index_internal_xqcc_letters_path, error: "Could not find the letter by ID: #{ params[:id] }" and return unless @letter
        preview_xqcc_misread_letter
    end
private
    def xqcc_letter_misread_params
        params.require(:xqcc_letter_misread).permit(:transaction_code, :amended_status, :xray_ref_no, :employer_ref_no, :xray_code, :xray_name, :doctor_code, :employer_code, :worker_code, :worker_name, :worker_passport, :xray_address, :doctor_address, :employer_address)
    end

    def preview_xqcc_misread_letter
        ...
        render pdf: "xqcc_misread_letter_#{ params[:type] }",
        template: "/internal/xqcc_letters/xqcc_misread_letter_#{ params[:type] }.html.erb",
        layout: "pdf.html",

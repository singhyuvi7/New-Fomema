module Internal
  class JobTypesController < InternalController
    before_action :set_job_type, only: [:show, :edit, :update, :destroy]

    # GET /job_types
    # GET /job_types.json
    def index
      @job_types = JobType.search_name(params[:name])
      .page(params[:page])
      .per(get_per)
    end
    ...
    def create
      ...
      respond_to do |format|
        if @job_type.save
          format.html { redirect_to internal_job_types_url, notice: 'Job type was successfully created.' }
    ...
    def update
      respond_to do |format|
        if @job_type.update(job_type_params)
          format.html { redirect_to internal_job_types_url, notice: 'Job type was successfully updated.' }
    ...
    def destroy
      @job_type.destroy
      respond_to do |format|
        format.html { redirect_to internal_job_types_url, notice: 'Job type was successfully destroyed.' }

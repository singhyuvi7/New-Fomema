class Internal::InsurancesController < InternalController
        skip_before_action :verify_authenticity_token, only: [:premium_calculated, :policy, :insurance_orders]
        skip_before_action :authenticate_internal_user!, only: [:policy, :insurance_orders]
        ...
        def submit_calculator
            ...
            @submit_data = InsuranceService.calculator_submit_data(order: @order, response_url: premium_calculated_internal_insurances_url, backend_url: premium_calculated_internal_insurances_url)
        ...
        def submit_paid_premium
            ...
            @submit_data = InsuranceService.paid_premium_submit_data(order: @order, response_url: policy_internal_insurances_url, backend_url: policy_internal_insurances_url)
        ...
        def insurance_orders
            provider_code = params[:provider_code]&.strip&.upcase || "HOWDEN"
            decrypted_data = InsuranceService.decrypt(params[:param])

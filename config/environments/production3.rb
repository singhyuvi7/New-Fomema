Rails.application.configure do
  ...
  # config.active_job.queue_name_prefix = "fomema_#{Rails.env}"

  config.action_mailer.perform_caching = false

  config.action_mailer.default_url_options = {
    subdomain: ENV['SUBDOMAIN_NIOS'].try(:downcase) || 'fomema-nios-uat',
    host: ENV['APP_DOMAIN'] || 'bookdoc.com'
  }
  ...
  config.active_storage.service = ENV["ACTIVE_STORAGE_SERVICE"] || :stagingali
  ...
  config.content_security_policy do |policy|
    policy.default_src("*  data: blob: filesystem: about: ws: wss: 'unsafe-inline' 'unsafe-eval'")
    policy.script_src("* data: blob: 'unsafe-inline' 'unsafe-eval'")
    policy.connect_src("* data: blob: 'unsafe-inline'")
    policy.img_src("* data: blob: 'unsafe-inline'")
    policy.frame_src("* data: blob: ")
    policy.style_src("* data: blob: 'unsafe-inline'")
    policy.font_src("* data: blob: 'unsafe-inline'")
    policy.object_src("* data: blob: 'unsafe-inline'")
    policy.frame_ancestors("*")
  end

  config.force_ssl = true

  config.ssl_options = { hsts: { expires: 1.day, subdomains: true } }
end